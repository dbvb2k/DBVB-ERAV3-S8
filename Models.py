...
class CIFAR10Net2(nn.Module):
    def __init__(self):
        ...
        self.c4 = nn.Sequential(
            nn.Conv2d(64, 84, kernel_size=3, padding=2, dilation=2, bias=False),  # Reduced from 128 to 84
            nn.BatchNorm2d(84),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Conv2d(84, 84, kernel_size=3, padding=2, dilation=2, bias=False),  # Reduced from 128 to 84
            # nn.BatchNorm2d(96),
            # nn.ReLU(),
            # nn.Dropout(0.1),
        )
        ...
        self.fc = nn.Linear(84, 10)  # Changed from 128 to 84
    ...
    def forward(self, x):
        ...
        x = self.c4(x)
        x = self.gap(x)
        x = x.view(-1, 84)  # Changed from 128 to 84
        x = self.fc(x)
